...
| `fold` | Always create safe navigation: `exists = xyz` + `xyz = { ... }` becomes `xyz? = { ... }` (comments may sit in between, and so may other conditions - as long as none of them uses `xyz`); the scope-less `has_owner = yes` guard is folded the same way into an `owner`/`space_owner` block. `exists = xyz` + `NOT = { xyz = { ... } }` and `OR = { NOT = { exists = xyz } xyz? = { NOT = { C... } } }` leftovers are folded too (see the notes). Targets Stellaris v4.4+. |
