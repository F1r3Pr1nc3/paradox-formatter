...
Inside `allow` blocks a trigger-side conditional is rewritten as the equivalent implication: `if = { limit = L body }` means "if `L` then `body`", which is exactly `OR = { NOT = { L } body }`. Conditional chains (`else_if`/`else`) keep their form, and so do conditionals outside `allow`. This is independent of the safe navigation setting.
...
* A `xyz? = { ... }` node is never negated (`exists AND ...` cannot be negated by flipping only its inner value), and trigger-side conditionals (`if = { limit = L body }`, i.e. `L implies body`) are never folded into `scope? = ...` - inside `allow` blocks they are rewritten as `OR = { NOT = { L } body }` instead (see section 4).
