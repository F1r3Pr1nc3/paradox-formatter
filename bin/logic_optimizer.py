...
def lowercase_keys(node_list):
	...
	KEYWORDS_TO_LOWER = {
		# Scopes
		'ROOT', 'PREV', 'FROM', 'FROMFROM', 'FROMFROMFROM', 'FROMFROMFROMFROM', 'THIS', 'Owner', 'Controller', # 'OWNER', 'CONTROLLER'
		# Flow Control & Commands
		'IF', 'ELSE', 'ELSE_IF', 'LIMIT', 'EFFECT', 'WHILE', 'BREAK', 'CONTINUE', 'SWITCH' # 'TRIGGER', 'MODIFIER', 'DEFAULT', 'FACTOR'
	}
...
EXPLICIT_LOGIC_KEYS = {'OR', 'NOR', 'NAND', 'NOT'}
...
def optimize_node_list(node_list, parent_key=None):
	...
	while i < len(node_list):
		# Look for the start of the pattern: a NOT or NOR node
		if i + 1 < len(node_list) and node_list[i].get('key') in ('NOT', 'NOR'):
			# Find next two non-comment nodes
			n2, n3 = None, None
			idx2, idx3 = -1, -1
			...
			if n2:
				...
				if temp_idx < len(node_list):
					...
					idx3 = temp_idx
			if n3 and n2.get('op') in ('<', '>', '<=', '>=', '=', '!=') and n3.get('key') in ('NOT', 'NOR'):
				v2 = n2.get('val', '')
				vo = n2.get('op')
				if v2 and isinstance(v2, str) and (vo != '=' or v2[0] == '@' or v2[-1].isdigit()):
					# Pattern found. Merge.
					n1 = node_list[i]
					# Negate comparison
					negated_op = {'<': '>=', '>': '<=', '<=': '>', '>=': '<', '=': '!=', '!=': '='}.get(vo)
	...
	while i < len(node_list):
		...
		# This conversion always requires a pre-existing 'NOT/NOR'
		if len(node_items) > 1 and any(n.get('key') in ('NOT', 'NOR') for n in node_items):
