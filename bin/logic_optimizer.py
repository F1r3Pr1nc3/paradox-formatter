    ...
    for node in node_list:
        if node['type'] == 'node':
            ...
            if 'key' in node and isinstance(node.get('val'), list):
                ...
                if (upper_key in KEYWORDS_TO_UPPER or original_key.startswith(KEYWORDS_TO_LOWER_START) or original_key.endswith(KEYWORDS_TO_LOWER_END)) and original_key != upper_key:
                    node['key'] = upper_key
                    changed = True
    ...
    for node in node_list:
        if node['type'] == 'node':
            ...
            if isinstance(val, list):
                ...
                print(f"ORIGINAL_VAL_KEY {val}: {node}")
                node['val'] = val.lower()
                changed = True
...
def _is_negation_node(node):
    if node['type'] != 'node':
        return False
    if node.get('key') in ('NOT', 'NOR', 'NAND') and isinstance(node.get('val'), list):
        return True
